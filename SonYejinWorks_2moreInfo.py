...
class SonYejin:
    ...
    def get_data(self, content):
        ...
        for movie in movies:
            ...
            casting.append(movie.xpath("./dl/dd/dl/dd[last()]/text()")[0])

        # get number of pages
        if self.total == 1000:
            self.total = int(html.xpath("//div[@class='paginator']/a[last()]/text()")[0])

        df = pd.DataFrame([titles,years,ratings,totalcomments,casting], index=['title','year','rating','total_comments','acted_by'])
        df = df.T
        return df

    def save_data(self,df):
        df.to_csv("SonYejin`s movies_2moreInfo.csv", encoding='utf-8-sig')
